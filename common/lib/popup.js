(function (w) {
  ...
  app.init = function () {
    ...
    app.preloader = $(
      "<div id='preloader'><img src='../img/spinner.gif'/></div>"
    );
    app.streamPreloader = $(
      "<div id='preloader-stream'><img src='../img/spinner.gif'/></div>"
    );
    app.btnPreloader = $(
      "<div id='preloader-btn'><img src='../img/spinner.gif'/></div>"
    );
    ...
    app.scroller.on("scroll", function () {
      if (
        app._scroller.scrollTop + app._scroller.offsetHeight >=
        app._scroller.scrollHeight - 200
      ) {
        if (
          app.curView &&
    ...
    $("body")
      ...
      .on("click", ".js-window", function (e) {
        ...
        utils.windows.create(
          $.extend({ url: $(this).attr("data-href") }, windowOpts)
        );
        e.preventDefault();
        window.close();
      });
  ...
  var DefaultView = Backbone.View.extend({
    ...
    render: function () {
      this.$el
        .empty()
        .html($(Handlebars.templates[this.template](this.model.toJSON())));
      return this;
    },
    onshow: function () {},
    onhide: function () {},
  ...
  var InfoView = DefaultView.extend({
    el: "#content",
    events: {
      "click #donate-btn": "openPaypal",
    },

    openPaypal: function () {
      utils.tabs.create({
        url: "https://www.paypal.com/donate/?hosted_button_id=UQHX9BP6QUH8G",
      });
    },
  });
  ...
  var SettingsView = LazyRenderView.extend({
    ...
    uploadSound: function (e) {
      utils.tabs.create({
        url: utils.runtime.getURL("common/html/upload.html"),
      });
    },

    playSound: function (e) {
      b.bgApp.playSound(
        b.settings.getNotificationSoundSource(),
        b.settings.get("notificationVolume").get("value") / 100
      );
    },
  ...
  var ListView = LazyRenderView.extend({
    ...
    showMessage: function (type) {
      if (this.messages[type]) {
        ...
        this.container
          .empty()
          .html(Handlebars.templates["screenmessage"](text));
      }
    },
    loadNext: function () {
  ...
  var ChannelNotificationListView = ListView.extend({
    ...
    toggle: function (e) {
      ...
      var val =
        $(e.currentTarget).data("notification-value") == "1" ? true : false;
      this.$el
    ...
    serialize: function () {
      var attributes = [];
      this.$el.find(".screen-content [data-channel-id]").map(function (i, e) {
        e = $(e);
        attributes.push({
          _id: parseInt(e.attr("data-channel-id")),
          notificationOpts: {
            desktop: e
              .find("input[data-notification-type='desktop']")
              .prop("checked"),
            sound: e
              .find("input[data-notification-type='sound']")
              .prop("checked"),
          },
        });
      });
      this.collection.set(attributes, {
        add: false,
        remove: false,
        silent: true,
      });
